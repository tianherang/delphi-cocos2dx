...
type
  ...
  private
    ...
    FNodes: TList<Pointer>;                 // These are the child elements
    ...
    function GetValueAsWidestring: string;
    procedure SetValueAsWidestring(const Value: string);
    ...
    function GetAttributeValueAsWidestring(Index: integer): string;
    ...
    procedure SetAttributeValueAsInteger(Index: integer;
      const Value: integer);
    function GetAttributeByNameWide(const AName: string): string;
  ...
  public
    ...
    // items to TXmlNode.
    procedure FindNodes(const NodeName: string; const AList: TList<Pointer>);
    ...
    // Use FromWidestring to convert string to a string for the node (name, value,
    // attributes). If the TNativeXml property UtfEncoded is True, all
    // character codes higher than $FF are preserved.
    function FromWidestring(const W: string): string;
    ...
    function IsEqualTo(ANode: TXmlNode; Options: TXmlCompareOptions; MismatchNodes: TList<Pointer> = nil): boolean;
    ...
    procedure NodesByName(const AName: string; const AList: TList<Pointer>);
    ...
    function ReadAttributeDateTime(const AName: string; ADefault: TDateTime = 0): TDateTime; virtual;
    ...
    // attribute is not found, or cannot be converted, the default ADefault will
    // be returned.
    function ReadAttributeInt64(const AName: string; ADefault: int64 = 0): int64; virtual;
    // Find the attribute with AName, and convert its value to a float. If the
    // attribute is not found, or cannot be converted, the default ADefault will
    // be returned.
    function ReadAttributeFloat(const AName: string; ADefault: double = 0): double;
    function ReadAttributeString(const AName: string; const ADefault: string = ''): string; virtual;
    ...
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadInt64(const AName: string; ADefault: int64 = 0): int64; virtual;
    ...
    // Read the subnode with AName and return its string value. If the subnode is
    // not found, ADefault will be returned.
    function ReadWidestring(const AName: string; const ADefault: string = ''): string; virtual;
    ...
    // to a string. If the TNativeXml property UtfEncoded is True, all
    // character codes higher than $FF are preserved.
    function ToWidestring(const s: string): string;
    ...
    procedure WriteAttributeDateTime(const AName:string; AValue: TDateTime; ADefault: TDateTime = 0); virtual;
    ...
    procedure WriteAttributeInt64(const AName: string; const AValue: int64; ADefault: int64 = 0); virtual;
    ...
    // will be added.
    procedure WriteAttributeString(const AName: string; const AValue: string; const ADefault: string = ''); virtual;
    ...
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteHex(const AName: string; AValue: integer; Digits: integer; ADefault: integer = 0); virtual;
    ...
    // Add or replace the subnode with AName and set its value to represent the string
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteWidestring(const AName: string; const AValue: string; const ADefault: string = ''); virtual;
    ...
    // as string. Set AttributeByNameWide to add an attribute to the attribute list, or replace an
    // existing one.
    property AttributeByNameWide[const AName: string]: string read GetAttributeByNameWide write
    ...
    // Read this property to get the string value of the attribute at index Index.
    // Write to it to set the string value.
    property AttributeValueAsWidestring[Index: integer]: string read GetAttributeValueAsWidestring write SetAttributeValueAsWidestring;
    ...
    // ValueAsWidestring returns the unescaped version of ValueDirect as a string.
    ...
    property ValueAsWidestring: string read GetValueAsWidestring write SetValueAsWidestring;
  ...
  // end;
  // </code>
  TXmlNodeList = class(TList<Pointer>)
  private
  ...
  public
    ...
    // string (without loss) or ToAnsiString to convert to ANSI string
...
// Convert a string to a UTF8 encoded string
function sdUnicodeToUtf8(const W: string): string;

// Convert a normal ansi string to a UTF8 encoded string
function sdAnsiToUtf8(const S: string): string;

// Convert a UTF8 encoded string to a string
function sdUtf8ToUnicode(const S: string): string;
...
begin
  ...
  if CharInSet(AValue[1], cQuoteChars) then
  begin
    P := PChar(AValue);
...
begin
  ...
  while i <= Length(AValue) do
    if CharInSet(AValue[i], cControlChars) then
      inc(i)
    else
    begin
      Result[j] := AValue[i];
...
begin
  ...
  begin
    ...
    begin
      if CharInSet(AValue[i], cQuoteChars) then
      begin
        InQuotes   := True;
        AQuoteChar := AValue[i];
      end;
  ...
  for i := Attributes.Count - 1 downto 1 do
    ...
    if CharInSet(Attributes[i][1], cQuoteChars) and (Pos('=', Attributes[i - 1]) > 0) then
    begin
      Attributes[i - 1] := Attributes[i - 1] + Attributes[i];
      Attributes.Delete(i);
    end;
...
begin
  ...
  while
    (Start < Close) and
    CharInSet(AValue[Start], cControlChars) do
    inc(Start);

  // Trim right
  while
    (Start < Close) and
    CharInSet(AValue[Close - 1], cControlChars) do
    dec(Close);
  ...
  try
    ...
    repeat
      ...
      begin
        ...
        begin
          if CharInSet(Ch, cQuoteChars) then
          begin
            InQuotes := true;
            QuoteChar := Ch;
          end;
...
begin
  ...
  //BinToHex(TBytes(Source), TBytes(Text), Count);
  Result := Text;
end;
...
procedure DecodeBinHexBuf(const Source: string; var Buffer; Count: Integer);
...
begin
  ...
  begin
    Ch := UpCase(AValue[1]);
    if CharInSet(Ch, ['T', 'Y']) then
    begin
      Result := True;
      exit;
    end;
    if CharInSet(Ch, ['F', 'N']) then
    begin
      Result := False;
      exit;
    end;
...
procedure TXmlNode.FindNodes(const NodeName: string; const AList: TList<Pointer>);
  // local
  procedure FindNodesRecursive(ANode: TXmlNode; AList: TList<Pointer>);
  var
    i: integer;
  begin
    with ANode do
...
function TXmlNode.FromWidestring(const W: string): string;
begin
  if Utf8Encoded then
...
function TXmlNode.GetAttributeByNameWide(const AName: string): string;
begin
...
function TXmlNode.GetAttributeValueAsWidestring(Index: integer): string;
begin
...
function TXmlNode.GetValueAsWidestring: string;
begin
...
function TXmlNode.IsEqualTo(ANode: TXmlNode; Options: TXmlCompareOptions;
  MismatchNodes: TList<Pointer>): boolean;
var
  i, Index: integer;
...
begin
  if (Index >= 0) and (Index < NodeCount) then
  begin
    TXmlNode(FNodes[Index]).DisposeOf;
    FNodes.Delete(Index);
  end;
end;
...
procedure TXmlNode.NodesByName(const AName: string; const AList: TList<Pointer>);
              ...
              begin
                for i := 0 to NodeCount - 1 do
                  ...
                  begin
                    ...
                    while (ClosePos > 0) and (CharInSet(Nodes[i].FValue[ClosePos], [#10, #13, ' '])) do
                      dec(ClosePos);
...
function TXmlNode.ReadWidestring(const AName: string; const ADefault: string): string;
begin
...
procedure TXmlNode.SetAttributeByNameWide(const AName: string; const Value: string);
begin
...
begin
  ...
  begin
    ...
    for i := 1 to length(Value) do
      if CharInSet(Value[i], cControlChars) then
...
procedure TXmlNode.SetValueAsWidestring(const Value: string);
begin
...
function TXmlNode.ToWidestring(const s: string): string;
begin
  if Utf8Encoded then
    Result := sdUtf8ToUnicode(s)
  else
...
procedure TXmlNode.WriteWidestring(const AName: string;
  const AValue: string; const ADefault: string);
begin
...
// under in the main NativeXml object
  ...
  begin
    repeat
      ...
      until not (CharInSet(Ch, cControlChars));
      if Ch = ']' then
        break;
      S.Seek(-1, soCurrent);
    until False;
...
procedure TsdAnsiStream.WriteBuf(const Buffer; Offset, Count: longint);
begin
  case FEncoding of
  se8Bit:
    begin
...
begin
  ...
  repeat
    ...
    if not (CharInSet(Ch, cControlchars)) then
      break;
  until False;
  Result := True;
end;
