...
type
  ...
  public
    ...
    destructor Destroy; override;
    {$IFNDEF UseDelphi}procedure LoadFromFile(const Filename: String); overload;{$ENDIF}
    {$IFNDEF UseDelphi}procedure LoadFromMemory(const pData: PByte; aSize: LongInt); overload;{$ENDIF}
...
function TStream.GetSize: Longint;
  ...
  procedure TMemoryStream.SetSize(const Value: Longint);
  begin
  ...
  function TMemoryStream.Read(var Buffer; Count: Longint): Cardinal;
  begin
    if fCurrent + Count <= fSize then
    begin
      Move(m_pData[fCurrent], Buffer, Count);
      Inc(fCurrent, Count);
      Result := Count;
    end else
    begin
  ...
  function TMemoryStream.Write(const Buffer; Count: Longint): Cardinal;
  begin
    if fCurrent + Count <= fSize then
    begin
      Move(Buffer, m_pData[fCurrent], Count);
      Inc(fCurrent, Count);
      Result := Count;
    end else
    begin
  ...
  constructor TMemoryStream.Create(pData: PByte; nSize: LongInt);
  begin
    inherited Create();
    m_pData := PByteArray(pData);
    fSize := nSize;
    fCurrent := 0;
  end;
...
{$IFNDEF UseDelphi}
  ...
  procedure TPngObject.LoadFromMemory(const pData: PByte; aSize: LongInt);
  var
    memStream: TMemoryStream;
  begin
    memStream := TMemoryStream.Create(pData, aSize);
...
function TPngObject.getData(): Pointer;
begin
  Result := Header.ImageData;
end;
